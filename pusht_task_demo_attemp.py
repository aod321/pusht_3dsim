...
#%%
def attempt_reach_pose(planner, reach_pose, env, max_attempts=5, axis=[0]):
    for attempt in range(max_attempts):
        res = planner.move_to_pose_with_RRTConnect(reach_pose)
        if res != -1:
            return res
        # Slightly adjust the goal pose if planning fails
        new_p = reach_pose.p
        for i, a in enumerate(axis):
            new_p[a] += np.random.uniform(-0.01, 0.01, size=1)
        reach_pose.set_p(new_p)
    return -1
...
while not viewer.closed:  # Press key q to quit
    ...
    if steps - last_step < 4:
        ...
        if viewer.window.key_down('i'):  # up
            ...
            res = attempt_reach_pose(planner, reach_pose, env, axis=[0])
            if res != -1:
            ...
            print(new_goal_pose)
            reach_pose = sapien.Pose(p=new_goal_pose.p, q=goal_pose.q)
            res = attempt_reach_pose(planner, reach_pose, env, axis=[1])
            if res != -1:
                create_maker_point(reach_pose, env)
                obs, reward, terminated, truncated, info = res
                print(reward)
            ...
            print(new_goal_pose)
            reach_pose = sapien.Pose(p=new_goal_pose.p, q=goal_pose.q)
            res = attempt_reach_pose(planner, reach_pose, env, axis=[0,1])
            if res != -1:
                create_maker_point(reach_pose, env)
                obs, reward, terminated, truncated, info = res
                print(reward)
